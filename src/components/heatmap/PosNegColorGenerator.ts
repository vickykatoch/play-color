export class PosnegColorGenerator {
    ...
    constructor(baseNegativeColor: string, basePositiveColor: string, incrementStep: number, colorCount: number = 20) {
        const posColors: string[] = [];
        const negColors: string[] = [];
        for (let i = 1; i <= colorCount; i++) {
            posColors.push(this.generateColors(basePositiveColor, i * incrementStep * 0.01));
            negColors.push(this.generateColors(baseNegativeColor, i * incrementStep * 0.01));
        }
    ...
    isWhiteColor(color: string): boolean {
        return ['#ffffff', '#fff'].includes(color.toLowerCase());
    }

    isBlackColor(color: string): boolean {
        return ['#000000', '#000'].includes(color.toLowerCase());
    }
